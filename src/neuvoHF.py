...
class NeuroevolutionTransformer:
    def __init__(self, data, type, evo_params=None, genotype=None, fittest=None,  
        ...
        self.model=model
        self.build_architecture(genotype_length, gene_value)
        return None

    def build_architecture(self, genotype_length=32, gene_value=40):
        '''
        ...
        '''
        self.build_ea(genotype_length=32, gene_value=40)
        # Choose a pre-trained tokenizer
        tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased")

        # Function to tokenize the input data (premise and hypothesis in this case)
        def tokenize_function(examples):
            return tokenizer(examples["premise"], examples["hypothesis"], truncation=True, padding="max_length", max_length=128)

        # Tokenize the dataset
        tokenized_dataset = self.data.map(tokenize_function, batched=True)
        # Step 4: Convert to PyTorch tensors
        tokenized_dataset.set_format(type='torch', columns=['input_ids', 'attention_mask', 'label'])

        self.train_mnli(tokenized_dataset)
    ...
    def build_ea(self, genotype_length=32, gene_value=40):
        ...
        if self.type.lower() == 'ga':
            self.EA = GA(shape=None, mutation_rate=self.mutation_rate, phenotype=self.genotype, eco=self.eco, transformer=True)
        else:
            self.EA = GE(shape=None, mutation_rate=self.mutation_rate, genotype=self.genotype,
                        genotype_length=genotype_length, gene_value=gene_value, user_grammar_file=self.grammar_file, transformer=True)
    ...
    def compute_metrics(self, batch):
        return {"wer": wer_metric.compute(predictions=[batch["transcription"]], references=[batch["text"]])}

    def train_mnli(self, dataset):
        lst_of_training_args = self.EA.phenotype
        # Define training arguments
        training_args = TrainingArguments(
            output_dir='./results',          # output directory
            num_train_epochs=lst_of_training_args.get('number of epochs'),              # number of training epochs
            per_device_train_batch_size=lst_of_training_args.get('batch size'),  # batch size for training
            per_device_eval_batch_size=lst_of_training_args.get('batch size'),   # batch size for evaluation
            warmup_steps=lst_of_training_args.get('warmup steps'),                # number of warmup steps for learning rate scheduler
            weight_decay=lst_of_training_args.get('weight decay'),               # strength of weight decay
            logging_dir='./logs',            # directory for storing logs
            logging_steps=10,
            learning_rate=lst_of_training_args.get('learning rate'),              # learning rate
            max_grad_norm=lst_of_training_args.get('max grad norm'),               # max gradient norm for gradient clipping
            save_total_limit=1,
        ...
        trainer = Trainer(
            ...
            eval_dataset=dataset['validation_matched']       # evaluation dataset
        )
        trainer.train()
    ...
    def run_whisper_model(self):
        ...
        model = WhisperForConditionalGeneration.from_pretrained(self.model)
        trainer = Trainer(
            model=model,                     # the instantiated 🤗 Transformers model to be trained
            args=training_args,              # training arguments, defined above
            train_dataset=self.data['train'],     # training dataset
            eval_dataset=self.data['test']       # evaluation dataset
        )
        trainer.train()
        result = trainer.evaluate()
...
class NeuvoBuilderTransformer():
    ...
    def __init__(self, evo_params=None, type='ga', fittest=None, eco=False, verbose=0, gene_value=40, genotype_length=32,
                 fitness_function='f1', grammar_file=None, model='openai/whisper-tiny.en'):
    ...
    def multi_train_whisper(self, population):
        '''
    ...
    def initialise_pop(self, insertions=[], elite_mode=False, grammar_file=None):
        ...
        with console.status("[bold green]Initialising population...") as status:
            ...
            for phenotype in insertions:
                a = NeuroevolutionTransformer(evo_params=self.parameter_list, data=self.data, phenotype=phenotype, type=self.type, fittest=None,
                                    eco=self.eco, verbose=self.verbose, gene_value=self.gene_value, genotype_length=self.genotype_length,
                                    grammar_file=self.grammar_file, model=self.model_name)
                pop.append(a)
            for _ in range(0, self.population_size-len(insertions)):
                a = NeuroevolutionTransformer(evo_params=self.parameter_list, data=self.data, type=self.type, eco=self.eco, verbose=self.verbose, fittest=None,
                                gene_value=self.gene_value, genotype_length=self.genotype_length, grammar_file=self.grammar_file, model=self.model_name)
                pop.append(a)

            self.population = self.multi_train_whisper(pop)
